...
import { Cross, User, Mail, Phone, Lock, Eye, EyeOff, Loader2, AlertCircle, ShieldCheck } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Doctor registration failed.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOAuthSignUp = (provider) => {
    // Simulate OAuth Login flow for Google / Microsoft
    setLoading(true);
    setTimeout(() => {
    ...
    <div className="min-h-screen flex flex-col justify-center items-center p-6 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight">Doctor Registration</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Create your secure Doctor Medicine Information Portal account</p>
        </div>
        ...
        <div className="glass-panel p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl space-y-5">
          ...
          <div className="relative flex items-center justify-center">
            ...
            <span className="bg-white dark:bg-slate-900 px-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider absolute">OR EMAIL SIGN UP</span>
          </div>

          {/* Standard Mandatory Note */}
          <div className="p-2.5 rounded-xl bg-medred-50/60 dark:bg-medred-950/30 border border-medred-100 dark:border-medred-900/40 text-left">
            <p className="text-[11px] text-slate-600 dark:text-slate-300 font-semibold leading-relaxed">
              <span className="text-medred-600 font-extrabold text-xs mr-0.5">*</span> Fields marked with a red asterisk (<span className="text-medred-600 font-bold">*</span>) are mandatory and must not be left empty.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">
                Doctor Full Name <span className="text-medred-600 font-extrabold">*</span>
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="e.g. Dr. Rupak Biswas"
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5">
                Contact Phone Number <span className="text-medred-600 font-extrabold">*</span>
              </label>
              <div className="relative">
          ...
          <div className="pt-4 border-t border-slate-200 dark:border-slate-800 text-center text-xs text-slate-500">
            Already registered?{' '}
            <Link to="/login" className="text-medred-600 dark:text-medred-400 font-bold hover:underline">
              Sign in to Portal
            </Link>
          </div>
        </div>
        ...
        <p className="text-center text-[11px] text-slate-400 mt-4 flex items-center justify-center gap-1">
          <ShieldCheck className="w-3.5 h-3.5 text-medred-600" /> HIPAA Compliant Doctor Authentication Engine
        </p>
      </div>
    </div>
  );
